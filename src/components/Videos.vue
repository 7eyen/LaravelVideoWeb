  ...
  <div>
    ...
    <swiper :options="swiperOption">
      <swiper-slide v-for="v in tags" :key="v.id">
        <router-link :to="{params:{tid:v.id},name:'Videos'}">{{v.name}}</router-link>
      </swiper-slide>
    </swiper>
    <!--导航条结束-->
    ...
    <ul id="videolist">
      <li v-for="v in lesson" :key="v.id">
        ...
        <router-link :to="{params:{lessonId:v.id,title:v.title},name:'Page'}">{{v.title}}</router-link>
      </li>
    </ul>
    <!--视频列表结束-->
...
export default {
  ...
  methods: {
    loadData() {
      ...
      this.axios.get("lesson/" + (tid ? tid : 1)).then(response => {
